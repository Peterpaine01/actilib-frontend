    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${process.env.EXPO_PUBLIC_SERVER_URL}/booking/${id}`
        );
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.container_bottom}>
        <View style={styles.section}>
          ...
          >
            <View>
              <Text style={styles.price}>{booking.activity.price} crédits</Text>
            </View>
            <TouchableHighlight style={styles.container_icon}>
              <Feather name="heart" size={24} />
            </TouchableHighlight>
          </View>
          <Text style={styles.title_accent}>Créneau réservé</Text>
        </View>
        <View style={styles.container_tag}>
          <Text style={styles.tag}>LIEU </Text>
        </View>
        <View style={[styles.container_place]}>
          {/* <Text style={styles.title}>{booking.activity.owner.name}</Text>
          <Text style={styles.text}>{booking.activity.owner.description}</Text> */}
        </View>
      </View>
    </ScrollView>
  );
